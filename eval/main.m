% Parameterize the script by dataset_name and save outputs under testresult_matlab/<dataset_name>
if ~exist('dataset_name','var') || isempty(dataset_name)
    % default dataset if not provided
    dataset_name = 'scape';
end
dataset_name_lower = lower(dataset_name);
out_base = fullfile('testresult_matlab', dataset_name);
if ~exist(out_base, 'dir')
    mkdir(out_base);
end

% enforce exact folder layout described by the user
mesh_dir = fullfile('visual_result', ['dvmatcher', dataset_name]);
feat_dir = fullfile('result', ['dvmatcher_', dataset_name], 'feature');
vts_dir = fullfile('data', dataset_name, 'corres');
geodist_dir_test = fullfile('data', dataset_name, 'geodist_test');
geodist_dir_train = fullfile('data', dataset_name, 'geodist_train');

% existence checks
if ~exist(mesh_dir, 'dir')
    error('Mesh folder not found: %s. Expected visual_result/dvmatcher<dataset_name>', mesh_dir);
end
if ~exist(feat_dir, 'dir')
    error('Feature folder not found: %s. Expected result/dvmatcher_<dataset_name>/feature', feat_dir);
end
if ~exist(vts_dir, 'dir')
    error('VTS folder not found: %s. Expected data/<dataset_name>/corres', vts_dir);
end
if ~exist(geodist_dir_test, 'dir') && ~exist(geodist_dir_train, 'dir')
    error('Geodesic (M) folder not found: expected data/<dataset_name>/geodist_test or geodist_train');
end

%% Full2Full
%% SCAPE
if contains(dataset_name_lower, 'scape')
    data_dir = fullfile('visual_result', ['dvmatcher', dataset_name]);
    if ~exist(data_dir, 'dir')
        % fallback to original testdata location
    ...
    vts_5k = {};
    for i = num_of_data
        vts_path = fullfile('data', 'scape', 'corres', sprintf('mesh%03d.vts', i));
        if exist(vts_path, 'file')
            vts = load(vts_path);
        else
            vts = [];
        end
    ...
    % load geodesic matrices
    M_path = fullfile('data', 'scape', 'M', 'M_scape_test.mat');
    if exist(M_path, 'file')
        M = load(M_path).M;
    else
        M = {};
    end
    errors1 = [];
    if contains(dataset_name_lower, 'dt4d')
        index{1} = 1:3;
        index{2} = 4:13;
        index{3} = 14:20;
        index{4} = 21:23;
        ...
        save(fullfile(out_base,'dt4d_full.mat'),'err');
    end
                arr(src-79,tar-79)=0;
                continue
            end
            ...
            Pidx = ones(siz,1);
            Partial_idx_S = ones(size(Pidx,1),1);
            phiS_path = fullfile(feat_dir, sprintf('usefeature_tr_reg_0%d.mat', src));
            phiT_path = fullfile(feat_dir, sprintf('usefeature_tr_reg_0%d.mat', tar));
            if exist(phiS_path,'file') && exist(phiT_path,'file')
                phiS = load(phiS_path).uphi;
                phiT = load(phiT_path).uphi;
            else
    ...
    save(fullfile(out_base,'faust_full.mat'),'arr','errors2','avg');
end
%% SHREC19_r
if contains(dataset_name_lower, 'shrec19')
    for i = 1:44
        M{i} = load(fullfile('data','shrec19_r','M', [num2str(i),'.mat'])).M;
    end
    data_dir = fullfile('data','shrec19_r','corres');
    files = dir(fullfile(data_dir, '*.map'));
    data_name_list = {files.name}';
    errors3 = [];
    for num = 1:length(data_name_list)
        originalStr = data_name_list{num};
        noSuffixStr = strrep(originalStr, '.map', '');
        splitStr = split(noSuffixStr, '_');
        src = splitStr{1};
        tar = splitStr{2};
        T_path = fullfile('testdata','cvpr25', ['Deformer_Uni3FC_lr_2e-3_bt_2_scape_conv_corr_shrec19_r'],'T', ['T_',noSuffixStr,'.txt']);
        if exist(T_path,'file')
            T = load(T_path);
        else
            T = [];
        end
        Tcorr = load(fullfile('data','shrec19_r','corres', [noSuffixStr,'.map']));
        splitStr = split(noSuffixStr, '_');
    ...
    errors3 = errors3(:);
    save(fullfile(out_base,'shrec19_full.mat'),'errors3');
end
%% SHREC07
...
if contains(dataset_name_lower, 'shrec07')
    for i = 1:20
        M{i} = load(fullfile('data','shrec07','M', ['M', num2str(i), '.mat'])).M;
    end
    errors2 = [];
    arr = [];
    for tar = 1:20
    ...
    errors2 = errors2(:);
    avg = calculateAverage(arr);
    save(fullfile(out_base,'shrec07_full.mat'),'arr','errors2','avg');
end
%% Full2Full
%% DT4D
index{1} = 1:3;
...
errors1 = [];
arr = [];
if contains(dataset_name_lower, 'topkids')
    data_dir = 'testdata/topkids/shapes/';
    files = dir(fullfile(data_dir, '*.off'));
    data_name_list = {files.name}';
    [V,Faces] = load_raw_data(data_dir,data_name_list);
    ...
    errors1 = errors1(:);
    save(fullfile(out_base,'topkids_full.mat'),'arr','errors1');
end
...
if contains(dataset_name_lower, 'scape_partial')
    data_dir = 'testdata/scape/shape/';
    files = dir(fullfile(data_dir, '*.off'));
    data_name_list = {files.name}';
    num_of_data = 52:71;
    ...
    for numview = 1:12
        ...
        for src = 52:71
            errors1 = [];
            idx_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_scape_partial',num2str(numview)],'index_partial',sprintf('index_mesh0%d.txt',src));
            if exist(idx_path,'file')
                indx = dlmread(idx_path)+1;
            else
                indx = [];
            end
            sz = size(indx);
            ...
            phiS_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_scape_partial',num2str(numview)],'feature',sprintf('usefeature_mesh0%d.mat',src));
            if exist(phiS_path,'file')
                phiS = load(phiS_path).uphi;
            else
                phiS = [];
            end
            phiT_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_scape_partial',num2str(numview)],'feature','usefeature_mesh000.mat');
            if exist(phiT_path,'file')
                phiT = load(phiT_path).uphi;
            else
                phiT = [];
            end
    ...
    save(fullfile(out_base,'scape_partial.mat'),'meanval');
end
...
if contains(dataset_name_lower, 'faust_partial')
    data_dir = 'testdata/faust/shape/shape_test';                       
    files = dir(fullfile(data_dir, '*.off'));
    data_name_list = {files.name}';
    num_of_data = 80:99;
    ...
    for numview = 1:12
        ...
        for src = 80:99
            errors1 = [];
            idx_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_faust_partial',num2str(numview)],'index_partial',sprintf('index_tr_reg_0%d.txt',src));
            if exist(idx_path,'file')
                indx = dlmread(idx_path)+1;
            else
                indx = [];
            ...
            phiS_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_faust_partial',num2str(numview)],'feature',sprintf('usefeature_tr_reg_0%d.mat',src));
            if exist(phiS_path,'file')
                tmp = load(phiS_path);
                phiS = tmp.uphi;
            else
            ...
            phiT_path = fullfile('testdata','cvpr25',['Deformer_Uni3FC_lr_2e-3_bt_5_scape_conv_partial_epoch20_faust_partial',num2str(numview)],'feature','usefeature_tr_reg_070.mat');
            if exist(phiT_path,'file')
                tmp = load(phiT_path);
                phiT = tmp.uphi;
            else
            ...
            geo_err = geo_err(:); 
            errors1  =  [errors1 ; geo_err];
            val{src-79} = mean(errors1);
            tal = tal + mean(errors1)*sz(1);
            num = num + sz(1);
        end
        meanval(numview) = tal/num;
    end
    save(fullfile(out_base,'faust_partial.mat'),'meanval');
end
%% SHREC19_r-Partial
for i = 1:44
...
if contains(dataset_name_lower, 'shrec19_partial')
    errors3 = errors3(:);
    save(fullfile(out_base,'shrec19_partial.mat'),'errors3');
else
    errors3 = errors3(:);
    % if not partial mode, still save the full variant if present
    if exist('errors3','var')
        save(fullfile(out_base,'shrec19_full.mat'),'errors3');
    end
end
