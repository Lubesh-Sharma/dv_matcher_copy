...
def deformation_graph_node(verts1):
    dg_list = []
    num_nodes_all = torch.tensor([]).to(verts1.device)
    for i in range(len(verts1)):
        geod = compute_geodesic_distmat(verts1[i].cpu().numpy())
...
def eval_net(cfg):
    ...
    # FAUST specific paths
    shape1_pth = "data/faust/shapes_train/tr_reg_000.off"
    shape2_pth = "data/faust/shapes_test/tr_reg_080.off"
    name1 = "tr_reg_000"
    name2 = "tr_reg_080"         
    with torch.no_grad():
        point_backbone_dino.eval() 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Launch the deformation for FAUST dataset.")
    parser.add_argument('--savedir', required=False, default="./data", help='root directory of the dataset')
    parser.add_argument("--config", type=str, default="deform_faust", help="Config file name")
