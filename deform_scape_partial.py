...
def eval_net(cfg):
    ...
    # For partial scape use partialexpname when available
    if cfg.get("isPartial", False):
        save_path = 'result/deform_partial_' + cfg.get("partialexpname", cfg.get("expname","scape_partial"))
        model_path = 'ckpt/' + cfg.get("partialexpname", cfg.get("expname","scape_partial"))+ '/ep_deformer_val_best.pth' 
        pointbackbone_path = 'ckpt/' + cfg.get("partialexpname", cfg.get("expname","scape_partial"))+ '/ep_val_best.pth' 
    else:
        save_path = 'result/deform_' + cfg.get("expname","scape_partial")
        model_path = 'ckpt/' + cfg.get("expname","scape_partial")+ '/ep_deformer_val_best.pth' 
        pointbackbone_path = 'ckpt/' + cfg.get("expname","scape_partial")+ '/ep_val_best.pth' 
    ...
    # scape_partial specific paths (partial dataset files are under data/scape_partial)
    shape1_pth = "data/scape_partial/shapes_train/mesh000.off"
    shape2_pth = "data/scape_partial/shapes_test/mesh053.off"
    name1 = "mesh000"
    name2 = "mesh053"         
    with torch.no_grad():
        point_backbone_dino.eval() 
        ...
        if cfg.get("deform_mesh", False):
            ...
            Pi_12 = knnsearch_t_grad(feat1, feat2, alpha=100)
            Pi_12 = topk_pi(Pi_12)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Launch the deformation demo for scape_partial dataset.")
    parser.add_argument('--savedir', required=False, default="./data", help='root directory of the dataset')
    parser.add_argument("--config", type=str, default="deform_scape_partial", help="Config file name for scape_partial")
